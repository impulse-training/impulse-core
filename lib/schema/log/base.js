...
function logBaseSchema(type) {
    return yup.object(Object.assign({ createdAt: timestamp_1.optionalTimestampSchema, updatedAt: timestamp_1.optionalTimestampSchema, profileId: yup.string().required(), audioFile: file_1.fileSchema.optional(), audioDurationSeconds: yup.number(), audioWaveform: yup.string(), text: yup.string(), type: yup.mixed().oneOf([type]).defined(), date: timestamp_1.timestampSchema, dateString: yup.string().required(), profileEmojiIDString: yup.string(), tacticId: yup.string().nullable(), tactic: yup.lazy((_value, options) => {
