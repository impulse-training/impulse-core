...
exports.issueSchema = yup.object().shape({
    ...
    recommendedStrategies: yup.array().of(firestore_1.documentReferenceSchema.required()),
    // For profile issues, we store references to the tactics that are the user's "reminder" for the
    // issue
    reminderTactics: yup.array().of(firestore_1.documentReferenceSchema.required()),
});
