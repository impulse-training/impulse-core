...
const schedulableRoutineSchema = routineBaseSchema.concat(yup.object().shape({
    type: yup
        .mixed()
        .oneOf(['time', 'location'])
        .required(),
...
const timeRoutineSchema = schedulableRoutineSchema.concat(yup.object().shape({
    type: yup.mixed().oneOf(['time']).required(),
}));
exports.timeRoutineSchema = timeRoutineSchema;
const dayDebriefRoutineSchema = routineBaseSchema.concat(yup.object().shape({
    type: yup.mixed().oneOf(['dayDebrief']).required(),
}));
exports.dayDebriefRoutineSchema = dayDebriefRoutineSchema;
const locationRoutineSchema = routineBaseSchema.concat(yup.object().shape({
